...
{
    return femu_l2p_get_maptbl_ent(ssd, lpn);
}

static inline struct ppa get_maptbl_ent_with_lat(struct ssd *ssd, uint64_t lpn,
                                                 uint64_t *meta_lat)
{
    return femu_l2p_get_maptbl_ent_with_lat(ssd, lpn, meta_lat);
}

static inline void set_maptbl_ent(struct ssd *ssd, uint64_t lpn, struct ppa *ppa)
{
    femu_l2p_set_maptbl_ent(ssd, lpn, ppa);
}

static inline uint64_t set_maptbl_ent_with_lat(struct ssd *ssd, uint64_t lpn,
                                               struct ppa *ppa)
{
    return femu_l2p_set_maptbl_ent_with_lat(ssd, lpn, ppa);
}
...
void ssd_init(FemuCtrl *n)
{
    ...
    femu_l2p_init_latency(ssd);
    femu_l2p_log_latency_config(ssd);
    ssd->l2p_stats_last_log_ns = qemu_clock_get_ns(QEMU_CLOCK_REALTIME);
    femu_l2p_init_l1_cache(ssd);
